...
@Repository
public interface PracticeCompletionRepository extends JpaRepository<PracticeCompletion, Integer> {
    @Query("SELECT COUNT(up) FROM PracticeCompletion up WHERE up.practice.practiceType = :practiceType AND up.isCompleted = true AND up.userId = :userId")
